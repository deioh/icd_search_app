...
input_category = sys.argv[4] if len(sys.argv) > 4 else None
...
with open(filename, newline='', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if code and desc:
            cursor.execute("INSERT OR IGNORE INTO icd_code (code, description, category) VALUES (?, ?, ?)", (code, desc, input_category))
            count += 1
